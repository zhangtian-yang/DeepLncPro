...
def motif_extraction(motif_sequences, layer1_parameters, layer1_out, motif_directory, tomtom_directory, stride=1):
    ...
    #                 shell=True)
    # subprocess.call('cp %s/tomtom_result/tomtom.tsv %s/tomtom_result/tomtom.txt' % (motif_directory, motif_directory),
    #                 shell=True)
    #
    # # read in annotations
    # filter_names = name_filters(filters_number, '%s/tomtom_result/tomtom.txt' % motif_directory,
    #                             'motif_database/JASPAR2018_CORE_vertebrates_non-redundant.meme')
    #
    # # print a table of information
    # table_out = open('%s/table.txt' % motif_directory, 'w')
    #
    ...
    #     consensus = filter_motif(filters[n, :, :])
    #
    #     # grab annotation
    #     annotation = '.'
    #     name_pieces = filter_names[n].split('_')
    #     if len(name_pieces) > 1:
    #         annotation = name_pieces[1]
    #
    #     # plot density of filter output scores
    #     fmean, fstd = plot_score_density(np.ravel(layer1_out[:, n, :]), '%s/filter%d_dens.pdf' % (motif_directory, n))
    #     # ???上一行n的位置
    #     row_cols = (n, consensus, annotation, filters_ic[n], fmean, fstd)
...
def just_motif_analysis(species, seq_len, encoding_type,model_path='xxx'):
    motif_directory = 'C:\\Users\\Zhang\\Desktop\\motif_analysis'
...
# deepncpro(args)
# motif分析
#just_motif_analysis('h', 181, 7,'D:\\Project\\deepnc\\DeepncPro_projects\\x_181_7_4\\bestmodel_1.pkl')
...
validation_data_m=validation_data


# m_train1=np.array(x1)
# m_train2=np.array(x2)
# print(np.shape(m_train1))
# print(np.shape(m_train2))
...
for i in range(len(test_data)):
    x3.append(test_data[i][0].flatten('F'))
    x4.append(test_data[i][1])
m_test1=np.array(x3)
m_test2=np.array(x4)
print(np.shape(m_test1))
print(np.shape(m_test2))
...
# x1,x2=[],[]
x1,x2=[],[]
for i in range(len(train_data_m)):
    x1.append(train_data_m[i][0].flatten('F'))
    x2.append(train_data_m[i][1])
for i in range(len(train_data)):
    x1.append(train_data[i][0].flatten('F'))
    x2.append(train_data[i][1])
# for i in range(len(validation_data_m)):
#     x1.append(validation_data_m[i][0].flatten('F'))
#     x2.append(validation_data_m[i][1])
# for i in range(len(validation_data)):
#     x1.append(validation_data[i][0].flatten('F'))
#     x2.append(validation_data[i][1])
m_train1=np.array(x1)
m_train2=np.array(x2)
print(np.shape(m_train1))
print(np.shape(m_train2))
...
x5,x6=[],[]
for i in range(len(test_data)):
    x5.append(test_data[i][0].flatten('F'))
    x6.append(test_data[i][1])
h_test1=np.array(x5)
h_test2=np.array(x6)
print(np.shape(h_test1))
print(np.shape(h_test2))
...
'''
# print('m')
# test_LinearSVC_C(m_train1, m_test1, m_train2, m_test2)
# print('h')
# test_LinearSVC_C(h_train1, h_test1, h_train2, h_test2)#结果5
'''
# 结果1
...
model1.fit(m_train1, m_train2)
m_pred1 =model1.predict_proba(m_test1)
#Predict Output
print(model1.score(m_test1, m_test2))
# model2= RandomForestClassifier()
# model2.fit(h_train1, h_train2)
#Predict Output
h_pred1 =model1.predict_proba(h_test1)
joblib.dump(model1, "ml_model/model1.m")

print('DecisionTree')
from sklearn.tree import DecisionTreeClassifier
model2= DecisionTreeClassifier()
# Train the model using the training sets and check score
model2.fit(m_train1, m_train2)
m_pred2 =model2.predict_proba(m_test1)
#Predict Output
print(model2.score(m_test1, m_test2))
# model2= DecisionTreeClassifier()
# model2.fit(h_train1, h_train2)
#Predict Output
h_pred2 =model2.predict_proba(h_test1)
joblib.dump(model2, "ml_model/model2.m")

print('Logistic')
from sklearn.linear_model import LogisticRegression
model3= LogisticRegression()
# Train the model using the training sets and check score
model3.fit(m_train1, m_train2)
m_pred3 =model3.predict_proba(m_test1)
#Predict Output
print(model3.score(m_test1, m_test2))
# model2= LogisticRegression()
# model2.fit(h_train1, h_train2)
#Predict Output
h_pred3 =model3.predict_proba(h_test1)
joblib.dump(model3, "ml_model/model3.m")

print('GaussianNB')
from sklearn.naive_bayes import GaussianNB
model4= GaussianNB()
# Train the model using the training sets and check score
model4.fit(m_train1, m_train2)
m_pred4 =model4.predict_proba(m_test1)
#Predict Output
print(model4.score(m_test1, m_test2))
# model2= GaussianNB()
# model2.fit(h_train1, h_train2)
#Predict Output
h_pred4 =model4.predict_proba(h_test1)
joblib.dump(model4, "ml_model/model4.m")

print('knn')
from sklearn.neighbors import KNeighborsClassifier
model5= KNeighborsClassifier(n_neighbors=100)
# Train the model using the training sets and check score
model5.fit(m_train1, m_train2)
m_pred5 =model5.predict_proba(m_test1)
#Predict Output
print(model5.score(m_test1, m_test2))
# model2= KNeighborsClassifier()
# model2.fit(h_train1, h_train2)
#Predict Output
h_pred5 =model5.predict_proba(h_test1)
np.set_printoptions(threshold=np.inf) # np.inf表示正无穷
# print('duandian',h_pred5)
joblib.dump(model5, "ml_model/model5.m")

print('svm')
from sklearn.svm import SVC
model6= SVC(probability=True)
# Train the model using the training sets and check score
model6.fit(m_train1, m_train2)
m_pred6 =model6.predict_proba(m_test1)
#Predict Output
print(model6.score(m_test1, m_test2))
# model2= SVC(probability=True)
# model2.fit(h_train1, h_train2)
#Predict Output
h_pred6 =model6.predict_proba(h_test1)
joblib.dump(model6, "ml_model/model6.m")

print('xgboost')
from xgboost import XGBClassifier
model7= XGBClassifier()
# Train the model using the training sets and check score
model7.fit(m_train1, m_train2)
m_pred7 =model7.predict_proba(m_test1)
#Predict Output
print(model7.score(m_test1, m_test2))
# model2= XGBClassifier()
# model2.fit(h_train1, h_train2)
#Predict Output
h_pred7 =model7.predict_proba(h_test1)
joblib.dump(model7, "ml_model/model7.m")

##########################################
#matrixs
print('-----------------------------------------------------------------------')
print('NAME species acc, sn, sp, mcc')
acc, sn, sp, mcc = model_metrics(m_test2, m_pred1[:,1].round())
print('1.RandomForest   m   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(h_test2, h_pred1[:,1].round())
print('1.RandomForest   h   ',acc, sn, sp, mcc)
# acc, sn, sp, mcc = model_metrics(m_test2, m_pred2[:,1].round())
# print('2.DecisionTree   m   ',acc, sn, sp, mcc)
# acc, sn, sp, mcc = model_metrics(h_test2, h_pred2[:,1].round())
# print('2.DecisionTree   h   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(m_test2, m_pred3[:,1].round())
print('3.Logistic   m   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(h_test2, h_pred3[:,1].round())
print('3.Logistic   h   ',acc, sn, sp, mcc)
# acc, sn, sp, mcc = model_metrics(m_test2, m_pred4[:,1].round())
# print('4.naive_bayes   m   ',acc, sn, sp, mcc)
# acc, sn, sp, mcc = model_metrics(h_test2, h_pred4[:,1].round())
# print('4.naive_bayes   h   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(m_test2, m_pred5[:,1].round())
print('5.Knn   m   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(h_test2, h_pred5[:,1].round())
print('5.Knn   h   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(m_test2, m_pred6[:,1].round())
print('6.SVM    m   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(h_test2, h_pred6[:,1].round())
print('6.SVM    h   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(m_test2, m_pred7[:,1].round())
print('7.Xgboost   m   ',acc, sn, sp, mcc)
acc, sn, sp, mcc = model_metrics(h_test2, h_pred7[:,1].round())
print('7.Xgboost   h   ',acc, sn, sp, mcc)
...
def load_dataset(train_data_path, validation_data_path, test_data_path,
                 train_data_path2, validation_data_path2, test_data_path2,
                 batch_size=32, encoding=7):
    """Create dataset and dataloader."""
    global train_dataset
    global validation_dataset
    global test_dataset
    ...
    global validation_dataloader
    global test_dataloader
    global test_dataloader1
    global test_dataloader2
    global motif_dataloader
    global motif_seq
    global base
    global DPCP
    ...
    # Create data object.
    train1 = open_data_path(train_data_path, encoding, base)
    train2 = open_data_path(train_data_path2, encoding, base)
    validation1 = open_data_path(validation_data_path, encoding, base)
    validation2 = open_data_path(validation_data_path2, encoding, base)
    train_data = train1+train2
    validation_data=validation1+validation2
    train_data = train_data + validation_data
    test1 = open_data_path(test_data_path, encoding, base)
    test2 = open_data_path(test_data_path2, encoding, base)
    ...
    train_dataset = MyDataset(train_data)
    validation_dataset = MyDataset(validation_data)
    test_dataset1 = MyDataset(test1)
    test_dataset2 = MyDataset(test2)
    test_dataset = MyDataset(test_data)
    motif_dataset = MyDataset(motif_data)
    ...
    test_dataloader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=100000, shuffle=False,
                                                  pin_memory=True)
    test_dataloader1 = torch.utils.data.DataLoader(dataset=test_dataset1, batch_size=100000, shuffle=False,
                                                  pin_memory=True)
    test_dataloader2 = torch.utils.data.DataLoader(dataset=test_dataset2, batch_size=100000, shuffle=False,
                                                  pin_memory=True)
    motif_dataloader = torch.utils.data.DataLoader(dataset=motif_dataset, batch_size=100000, shuffle=False)
...
def filters_weblogo(filter_out, filters_lenth, motif_sequences, filters_directory, n):
    ...
    if filter_count > 0:
        ...
        weblogo_data = weblogo.LogoData.from_seqs(weblogo_seqs)
        weblogo_options = weblogo.LogoOptions()
        # weblogo_options.fineprint = 'Python plot'
        weblogo_format = weblogo.LogoFormat(weblogo_data, weblogo_options)
        weblogo_eps = weblogo.eps_formatter(weblogo_data, weblogo_format)
        with open('%s.eps' % out_path, 'wb') as weblogo_f:
            weblogo_f.write(weblogo_eps)
...
class Config:
    ...
    def __init__(self, sequence_lenth, encoding, batch_size):
        ...
        self.encoding = encoding
        self.input_channel = input_size_dic[encoding]
        self.out_channel1 = 24
        self.out_channel2 = 24
        self.filter_size = 10
        self.filter_size2 = 10
        self.stride = 1
        self.fc1_size = 48
...
species1='m'
species2 = 'h'
train_data_path = './DataSet/%sbp/%s_train_%s.csv' % (str(seq_len), species1, str(seq_len))
validation_data_path = './DataSet/%sbp/%s_validation_%s.csv' % (str(seq_len), species1, str(seq_len))
test_data_path = './DataSet/%sbp/%s_test_%s.csv' % (str(seq_len), species1, str(seq_len))
train_data_path2 = './DataSet/%sbp/%s_train_%s.csv' % (str(seq_len), species2, str(seq_len))
validation_data_path2 = './DataSet/%sbp/%s_validation_%s.csv' % (str(seq_len), species2, str(seq_len))
test_data_path2 = './DataSet/%sbp/%s_test_%s.csv' % (str(seq_len), species2, str(seq_len))
tomtom_dir = './meme-5.3.3/src/tomtom'

# data
# make_dir(species, seq_len, encoding_type, None)
load_dataset(train_data_path, validation_data_path, test_data_path,
             train_data_path2, validation_data_path2, test_data_path2,
             batch_size=batch_size, encoding=encoding_type)
# record = Record(species, seq_len, encoding_type)
#
model=torch.load('D:\\Project\\deepnc\\\DDD\\bestmodel_0.pkl').to(device)
with torch.no_grad():
    model.eval()
    for i, (data, target) in enumerate(test_dataloader1):
        data = data.to(device)
        target = target.to(device)
        output = model(data)
        pred = output.detach().cpu().numpy().reshape(output.shape[0])
        ...
        acc, sn, sp, mcc = model_metrics(labels, pred.round())
        auc = metrics.roc_auc_score(labels, pred)
l1 = copy.deepcopy(labels)
p1 = copy.deepcopy(pred)
print(acc, sn, sp, mcc, auc)
with torch.no_grad():
    model.eval()
    for i, (data, target) in enumerate(test_dataloader2):
        data = data.to(device)
        target = target.to(device)
        output = model(data)
        pred = output.detach().cpu().numpy().reshape(output.shape[0])
...
def just_motif_analysis(species, seq_len, encoding_type, model_path='xxx'):
    motif_directory = 'C:\\Users\\Zhang\\Desktop\\motif_analysis'
    tomtom_directory = 'xxx'
    model_path = './DeepncPro_projects_2021.12.03一层卷积/model/' + species + '_' + str(seq_len) + '_' + str(
        encoding_type) + '/bestmodel_0.pkl'
    batch_size = 32
    train_data_path = './DataSet/%sbp/%s_train_%s.csv' % (str(seq_len), species, str(seq_len))
    validation_data_path = './DataSet/%sbp/%s_validation_%s.csv' % (str(seq_len), species, str(seq_len))
    test_data_path = './DataSet/%sbp/%s_test_%s.csv' % (str(seq_len), species, str(seq_len))
...
def plot_roc(name, labels, predictions, **kwargs):
    ...
    fp, tp, _ = metrics.roc_curve(labels, predictions, pos_label=1)
    plt.plot(fp, tp, linewidth=1.5, label='%s(AUROC = %0.3f)' % (name, roc_auc), **kwargs)
    plt.ylabel('True Positive Rate', fontdict=font)
    plt.xlabel('False Positive Rate', fontdict=font)
    plt.xlim([0.0, 1.01])
    plt.ylim([0.0, 1.01])
    plt.title('Mouse', fontdict=font3)
    plt.grid(True)
    ax = plt.gca()
    ax.set_aspect('equal')
    # ROC曲线
...
colors = ['indianred', 'darkcyan','indigo','orange','deeppink','royalblue']

plot_roc("RandomForest", m_test2, m_pred1[:,1], color=colors[0])
plot_roc("LogisticRegression", m_test2, m_pred3[:,1], color=colors[1])
plot_roc("KNN", m_test2, m_pred5[:,1], color=colors[2])
plot_roc("SVM", m_test2, m_pred6[:,1], color=colors[3])
plot_roc("XGBoost", m_test2, m_pred7[:,1], color=colors[4])
plot_roc("DeepLncPro", l1, p1, color=colors[5])


plt.plot([0, 1], [0, 1], color='black', lw=0.5, label='Chance')
plt.legend(loc='lower right', prop=font2)
plt.savefig("C:\\Users\\Zhang\\Desktop\\picture\\ml_roc_mouse.svg", dpi=600)
...
def plot_roc(name, labels, predictions, **kwargs):
    ...
    plt.xlim([0.0, 1.01])
    plt.ylim([0.0, 1.01])
    plt.title('Human', fontdict=font3)
    plt.grid(True)
    ax = plt.gca()
    ax.set_aspect('equal')
    # ROC曲线
...
plot_roc("RandomForest", h_test2, h_pred1[:,1], color=colors[0])
plot_roc("LogisticRegression", h_test2, h_pred3[:,1], color=colors[1])
plot_roc("KNN", h_test2, h_pred5[:,1], color=colors[2])
plot_roc("SVM", h_test2, h_pred6[:,1], color=colors[3])
plot_roc("XGBoost", h_test2, h_pred7[:,1], color=colors[4])
plot_roc("DeepLncPro", l2, p2, color=colors[5])

plt.plot([0, 1], [0, 1], color='black', lw=0.5, label='Chance')
plt.legend(loc='lower right', prop=font2)
plt.savefig("C:\\Users\\Zhang\\Desktop\\picture\\ml_roc_human.svg", dpi=600)
